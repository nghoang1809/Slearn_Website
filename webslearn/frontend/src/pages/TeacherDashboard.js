            ...
            {courses.map((course) => (
              <div key={course.id} className="bg-gradient-to-br from-gray-800 to-gray-900 rounded-xl p-6 hover:from-gray-750 hover:to-gray-800 transition-all duration-300 border border-gray-700 hover:border-teal-500 hover:shadow-lg hover:shadow-teal-500/20">
                ...
                <div className="flex flex-wrap gap-2 border-t border-gray-700 pt-4">
                  ...
                  <button
                    onClick={() => navigate(`/courses/${course.id}/manage-lessons`)}
                    className="flex-1 bg-yellow-500/20 hover:bg-yellow-500/30 text-yellow-400 px-3 py-2 rounded-lg text-sm font-medium transition-colors flex items-center justify-center gap-2"
                  >
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5h6M11 9h6M5 7h.01M5 11h.01M5 15h.01M11 13h6" />
                    </svg>
                    Quản lý
                  </button>
                </div>
